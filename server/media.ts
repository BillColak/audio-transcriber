...
export class FfmpegMedia {
  ...
  async prepare(inputPath: string, jobId: string): Promise<PreparedChunk[]> {
    ...
    await run(ffmpegExecutable, ['-y', '-i', inputPath, '-vn', '-ac', '1', '-ar', '16000', '-b:a', '64k', '-f', 'segment', '-segment_time', '1200', '-reset_timestamps', '1', pattern], { maxBuffer: 10 * 1024 * 1024 });
  ...
  private async duration(file: string): Promise<number> {
    if (!ffmpegExecutable) throw new Error('FFmpeg binary is unavailable.');
    try { await run(ffmpegExecutable, ['-i', file], { maxBuffer: 1024 * 1024 }); return 0; }
    catch (error) {
